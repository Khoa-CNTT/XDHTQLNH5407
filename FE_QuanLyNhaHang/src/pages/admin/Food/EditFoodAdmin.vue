            ...
            <div class="w-[70vw] h-full flex justify-center items-start text-xl rounded-lg">
                <div class="w-full h-[70vh] flex flex-row">
                    ...
                    <div class="border flex flex-1 gap-2 flex-col items-center p-2 rounded-e-lg">
                        ...
                        <div class="w-[300px]">
                            <p>Kiểu món ăn</p>
                            <select class="h-10 w-full border-2 ps-2" v-model="foodData.type" name="type" id="type">
...
if (route.query.data) {
    ...
    Object.assign(foodData.value, {
        ...
        best_seller: parsedData.bestSeller, // Đã sửa
...
function toggleCategory(categoryId) {
    if (foodData.value.categories.includes(categoryId)) {
        foodData.value.categories = foodData.value.categories.filter(id => id !== categoryId);
    } else {
...
async function goSave() {
    try {
        const response = await axios.put(`http://127.0.0.1:8000/api/admin/foods/${foodData.value.id}`, {
            name: foodData.value.name,
            cost: foodData.value.cost,
            best_seller: foodData.value.best_seller,
            status: foodData.value.status,
            detail: foodData.value.detail,
            type: foodData.value.type,
            categories: foodData.value.categories,
            // id_category_food:
        });
