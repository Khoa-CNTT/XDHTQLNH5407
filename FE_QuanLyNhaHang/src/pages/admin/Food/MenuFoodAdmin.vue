                  ...
                  <div class="overflow-hidden flex flex-row justify-center items-center">
                    ...
                    <div class="ps-5 flex flex-col gap-5">
                      ...
                      <div class="flex flex-row gap-2 items-center">
                        ...
                        <BestSellerSwitch :model-value="item.bestSeller"
                          @toggle="(newVal) => toggleBestSeller(item, newVal)" />
...
async function toggleBestSeller(item, newVal) {
  try {
    await axios.put(`http://127.0.0.1:8000/api/admin/foods/${item.id}`, {
      ...item,
      bestSeller: newVal
    ...
    console.error("Không thể cập nhật bestSeller:", error)
    alert("Cập nhật bestSeller thất bại.")
  }
}
  ...
  try {
    ...
    if (Array.isArray(response.data)) {
      allItems.value = response.data.map(item => ({
        ...
        detail: item.detail,
        status: item.status,
        type: item.type,             // { id, name }
        categories: item.categories, // Array<{ id, name }>
      }));
    } else {
...
async function confirmDelete() {
  ...
  try {
    await axios.delete(`http://127.0.0.1:8000/api/admin/foods/${itemToDelete.value.id}`)
